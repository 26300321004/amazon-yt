      ...
      <div className="w-full py-3 bg-amazon_blue  text-white px-4   flex items-center gap-4">
        ...
        <div className="h-10 rounded-md flex flex-grow relative hidden mdl:flex">
          <span
            ...
            className="w-14 h-full bg-gray-200 hover:bg-gray-300 border-2 cursor-pointer duration-300 text-sm text-amazon_blue font-serif flex items-center justify-center rounded-tl-md rounded-bl-md"
          >
            All{" "}
            <span>
          ...
          {showAll && (
            <div>
              <ul className="absolute w-56 h-80 top-10 left-0 overflow-y-scroll overflow-x-hidden bg-green-200 border-[1px] rounded-md border-rose-400 text-rose-400 p-2 flex flex-col gap-1 z-50">
                {allItems.map((item) => (
                  <li
                    className="text-sm tracking-wide border-b-[2px] border-b-transparent hover:border-red-400 cursor-pointer duration-200"
                    key={item._id}
                  >
                    {item.title}
                  </li>
        ...
        <Link to="/cart">
          <div className="flex items-start justify-center headerHover relative">
            ...
            <p className="text-xs font-semibold mt-3 text-whiteText">
              Cart{" "}
              <span className="absolute text-xs top-0 left-6 font-semibold p-1 h-4 bg-[#f3a847] text-rose-600 flex items-center justify-center rounded-full">
        ...
        {userInfo && (
          ...
          >
            <LogoutIcon />
            <p className="hidden mdl:inline-flex text-xs font-serif text-white">
              Log out
            </p>
          </div>
        )}
